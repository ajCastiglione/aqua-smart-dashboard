...
import { formatFullName, formatInstallDate, formatStreetAddress } from '../utils/format'
import {
  latestFlowReading,
...
export const CustomerDetailPage = () => {
  ...
  const poolTempFahrenheit = parseNumericString(latestTemperatureRow?.Temp_Reading)
    ...
    <div className="flex flex-col gap-6 lg:flex-row">
      <aside className="w-full shrink-0 space-y-4 lg:max-w-sm">
        <Card>
          ...
          <p className="mt-1 text-lg font-bold text-slate-900">{formatFullName(profile)}</p>
          <p className="mt-2 text-sm text-slate-600">
            Service tech: <span className="font-medium text-slate-800">{profile.TechName}</span>
          </p>
        ...
        <Card title="Pool & equipment">
          <p className="text-sm font-medium text-slate-800">{formatStreetAddress(profile)}</p>
          <p className="mt-2 text-sm text-slate-600">
            Phone: <span className="text-slate-900">{profile.PhoneNumber}</span>
          </p>
          <p className="mt-1 text-sm text-slate-600">
            Model / serial:{' '}
            <span className="font-mono text-slate-900">{profile.SerialNumber}</span>
          </p>
          <p className="mt-1 text-sm text-slate-600">
            Installed:{' '}
            <span className="text-slate-900">{formatInstallDate(profile.InstallDate)}</span>
          </p>
        ...
        {vendorQuery.data ? (
          <Card title="Vendor">
            <p className="font-medium text-slate-800">{vendorQuery.data.VendorName}</p>
            <p className="mt-1 text-sm text-slate-600">
              {vendorQuery.data.Street}, {vendorQuery.data.City}, {vendorQuery.data.State}{' '}
              {vendorQuery.data.ZipCode}
            </p>
          </Card>
        ) : null}
      </aside>
      ...
      <section className="min-w-0 flex-1 rounded-2xl border border-white/10 bg-[#0a1a2e]/90 p-4 shadow-xl backdrop-blur-md md:p-6">
        <div className="flex flex-col gap-4 border-b border-white/10 pb-4 md:flex-row">
          <nav className="flex flex-col gap-1 md:flex-row md:gap-2" aria-label="Customer sections">
            {tabs.map((tabItem) => (
              <button
                key={tabItem.id}
                type="button"
        ...
        <div className="pt-6">
          {tab === 'info' ? (
            <div className="space-y-6">
              <div>
                <p className="text-sm font-medium text-sky-200/80">Pool temperature</p>
                <p className="text-4xl font-bold text-white md:text-5xl">
                  {poolTempFahrenheit === undefined ? '—' : `${poolTempFahrenheit}°`}
                  <span className="ml-2 text-2xl font-normal text-sky-300">Pool Temp</span>
                </p>
              </div>

              {tempQuery.isError ? (
                <ErrorMessage
                  message={getErrorMessage(tempQuery.error)}
              ...
              <div className="rounded-xl border border-dashed border-white/20 bg-slate-900/40 p-4">
                <p className="text-sm font-medium text-sky-200">Cleaning / runtime</p>
                <p className="mt-2 text-sm text-sky-300/90">
                  Live completion percentage is not provided by the API. See the Servicing tab for
                  scheduled pump windows.
                </p>
              </div>
            </div>
          ) : null}
